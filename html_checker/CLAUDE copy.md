...
**バージョン競合の防止策**:
1. **基本方針: PR作成時にバージョン更新**
   - 機能完成時に`manifest.json`のバージョンを更新
   - コミットメッセージに「version: X.X.X」を明記
   - PR説明にバージョン変更理由を記載
   
2. **競合発生時の調整**
   - 複数PR同時進行時は、後からマージするPRがバージョンを調整
   - マージ前に最新mainを確認し、必要に応じてバージョン番号を変更
   - 例: 先行PR→1.0.1、後続PR→1.0.1を1.0.2に調整

3. **自動化との統合**
   - Claude自動化範囲にバージョン更新を含める
   - Issue対応完了時に適切なバージョン更新を自動実行
   - 品質チェック前にバージョン更新を完了
...
3. **品質保証済みPR作成**
   - 最終品質チェック通過確認
   - 日本語コミットメッセージでcommit（version: X.X.X を含める）
   - リモートブランチへpush
