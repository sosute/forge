...
export function analyzeHeadingStructure(targetElements = null) {
  ...
  const headings = targetElements
    ? targetElements.headings
    : {
        all: Array.from(
          document.querySelectorAll('h1, h2, h3, h4, h5, h6')
        ).filter(h => !isExcludedElement(h)),
        h1: Array.from(document.querySelectorAll('h1')).filter(
          h => !isExcludedElement(h)
        ),
        h2: Array.from(document.querySelectorAll('h2')).filter(
          h => !isExcludedElement(h)
        ),
        h3: Array.from(document.querySelectorAll('h3')).filter(
          h => !isExcludedElement(h)
        ),
        h4: Array.from(document.querySelectorAll('h4')).filter(
          h => !isExcludedElement(h)
        ),
        h5: Array.from(document.querySelectorAll('h5')).filter(
          h => !isExcludedElement(h)
        ),
        h6: Array.from(document.querySelectorAll('h6')).filter(
          h => !isExcludedElement(h)
        ),
      };
...
export function detectHeadingIssues(targetElements = null) {
  ...
  const h1Elements = targetElements
    ? targetElements.headings.h1
    : Array.from(document.querySelectorAll('h1')).filter(
        h => !isExcludedElement(h)
      );
  debugLog(
    'Checker',
    'H1 elements found (excluding excluded elements):',
    h1Elements.length
  );
  ...
  if (h1Elements.length === 0) {
    issues.push({
      ...
      name: 'H1タグの欠落',
      message:
        'ページにH1タグがありません。H1はページのメインタイトルを表し、SEOとアクセシビリティの観点から必須です。',
      elements: [],
  ...
  const headingStructureIssues =
    analyzeDetailedHeadingStructure(targetElements);
...
function analyzeDetailedHeadingStructure(targetElements = null) {
  ...
  headings.forEach((heading, index) => {
    ...
    debugLog(
      'Checker',
      `Processing heading ${index + 1}: H${level} "${headingText}" (previous: H${previousLevel})`
    );
    ...
    if (!hasVisibleText && !isImageOnlyH1) {
      ...
      if (heading.children.length > 0) {
        const childrenText = Array.from(heading.children)
        ...
        if (childrenText.length === 0) {
          ...
          headingStructureProblems.push({
            ...
            suggestion:
              'この見出し要素を削除するか、適切なテキストを追加してください',
          });
        }
      } else {
        // 子要素がなく、テキストもない場合
    ...
    if (isImageOnlyH1) {
      ...
      headingStructureProblems.push({
        ...
        text: `実際のHTML:\n${getElementFullHTML(heading)}`,
        suggestion:
          '画像のH1をテキストベースのH1に変更することを推奨します：<h1>阪急ビューティーオンライン</h1>\n画像は<img>タグとして別途配置してください。',
      });
    }
    ...
    const hasMultipleSameTypeTags =
      childTags.filter(tag => tag === 'SPAN').length >= 5 || // 5つ以上のSPAN
      childTags.filter(tag => tag === 'DIV').length >= 3; // 3つ以上のDIV

    // 単純なタグは除外
    const hasOnlySimpleTags =
      childTags.length > 0 &&
      childTags.every(tag => ALLOWED_SIMPLE_TAGS.includes(tag));

    // 単純なタグのみの場合は複雑な構造として扱わない
    const isActuallyComplex =
      (hasDeepNesting || hasTooManyChildren || hasMultipleSameTypeTags) &&
      !hasOnlySimpleTags;
    ...
    debugLog(
      'Checker',
      `- SPAN count: ${childTags.filter(tag => tag === 'SPAN').length}`
    );
    debugLog(
      'Checker',
      `- DIV count: ${childTags.filter(tag => tag === 'DIV').length}`
    );
    debugLog('Checker', `- Deep nesting check result: ${hasDeepNesting}`);
    debugLog('Checker', `- Too many children: ${hasTooManyChildren}`);
    debugLog(
      'Checker',
      `- Multiple same type tags: ${hasMultipleSameTypeTags}`
    );
    ...
    if (isActuallyComplex) {
      ...
      debugLog(
        'Checker',
        `- Deep nesting: ${hasDeepNesting}, Many children: ${hasTooManyChildren}, Multiple same tags: ${hasMultipleSameTypeTags}`
      );
    ...
    if (
      ...
      headingStructureProblems.push({
        ...
        text: `実際のHTML:\n${getElementFullHTML(heading)}`,
        suggestion:
          'フッターのロゴはdivやspanに変更し、ページ本文のメインタイトルにH1を使用してください',
      });
    }
    ...
    if (
      ...
      const firstH1Index = Array.from(headings).findIndex(
        h => parseInt(h.tagName.charAt(1)) === 1
      );
    ...
    if (hasVisibleText || isImageOnlyH1) {
      ...
      if (isImageOnlyH1) {
        debugLog(
          'Checker',
          `Image-only H1 detected, setting previousLevel to ${level} for proper hierarchy`
        );
      }
    }
  });
  ...
  debugLog(
    'Checker',
    `Heading analysis complete. Problematic headings: ${problematicHeadings.length}`
  );
  debugLog(
    'Checker',
    'Problematic headings:',
    problematicHeadings.map(h => ({
      tag: h.tagName,
...
function formatHeadingStructureSolution(problems) {
  ...
  if (problemsByRule.emptyHeading.length > 0) {
    solutionText += formatProblemSection(
      '空の見出しタグ',
      problemsByRule.emptyHeading,
