...
function classifySemanticElements(element, semanticElements) {
  ...
  if (tagName === 'div') {
    ...
    if (
      text.match(
        /\d{4}[年\-/]\d{1,2}[月\-/]\d{1,2}日?|\d{1,2}[月\-/]\d{1,2}日?[,\s]*\d{4}/
      )
    ) {
...
function logPreprocessingStats(stats, targetElements, processingTime) {
  ...
  debugLog(
    'Preprocessor',
    `Headings: ${targetElements.headings.all.length} total`
  );
  ...
  debugLog(
    'Preprocessor',
    `Images: ${targetElements.accessibility.images.length} (${targetElements.accessibility.imagesWithoutAlt.length} without alt)`
  );
  debugLog(
    'Preprocessor',
  ...
  debugLog(
    'Preprocessor',
    `Form elements: ${targetElements.accessibility.formElements.length}`
  );
  debugLog(
    'Preprocessor',
    `Elements with role: ${targetElements.accessibility.elementsWithRole.length}`
  );
...
export function getExclusionStats() {
  ...
  Array.from(allElements).forEach(element => {
    if (isExcludedElement(element)) {
      ...
      // 除外理由を分類
      if (
        element.closest('.mc_sp-header-wrapper') ||
        element.closest('.mc_pc-header-wrapper')
      ) {
        excludedByReason.header++;
      } else if (
        element.closest('.mc_pc-footer-wrapper') ||
        element.closest('.mc_sp-footer-wrapper')
      ) {
